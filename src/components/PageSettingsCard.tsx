...
interface PageSettingsCardProps {
  pageName: string;
  hideCard?: boolean;
  shouldAcceptSubtitle?: boolean;
  shouldAcceptCtaText?: boolean;
}
...
export function PageSettingsCard({
  pageName,
  hideCard = false,
  shouldAcceptSubtitle = true,
  shouldAcceptCtaText = false,
}: PageSettingsCardProps) {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(
        `${pageName.charAt(0).toUpperCase() + pageName.slice(1)} page settings saved!`,
      );
    },
    onError: (err: Error) => {
    ...
    <div className="p-6 space-y-6">
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {shouldAcceptSubtitle && (
            <div className="space-y-2 group">
              <Label
                htmlFor="page_subtitle"
                className="flex items-center gap-2 font-semibold text-slate-700 text-sm"
              >
  ...
  return <Card className="border-none shadow-sm bg-white mb-8">{content}</Card>;
}
